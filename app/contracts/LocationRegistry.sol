...
contract LocationRegistry {
    ...
    Coord[][180000][360000] coordsIndex;
    ...
    function getIndexPoint(uint x, uint y) constant returns (uint, uint) {
        uint div = 10 ** (maxDecimals - indexDecimals);
        return (x / div, y / div);
    }
    ...
    function indexPoint(Coord pos) internal {
        uint div = 10 ** (maxDecimals - indexDecimals);
        coordsIndex[pos.x / div][pos.y / div].push(pos);
    }
    ...
    function getIndexArray(uint x, uint y) constant internal returns (Coord[]) {
        return coordsIndex[x][y];
    }
